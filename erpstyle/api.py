...
@frappe.whitelist(allow_guest=True)
def add_instance(name, email, telephone, recaptcha_challenge_field,recaptcha_response_field):
...
def install_instance(code):
    ...
    if target:
        ...
        new_target['activation_code'] = None
...
def redirect_invalid_users(fn):
    def innerfn(context):
        ...
        if user_doc.user_type == u'Website User':
            ...
            if has_supplier:
                if not frappe.request.path.startswith("/portal"):
                ...
                if not job_applicant and not frappe.request.path.startswith("/portal"):
                    frappe.local.flags.redirect_location = "/portal"
                    raise frappe.Redirect
